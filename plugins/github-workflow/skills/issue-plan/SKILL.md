...
6. If the selected artifact directory is under `<temp-dir>/deep-dive/` and the `deep-research`,
   ...
   - After `deep-research`, publish the research artifact through step 8 as a Research Phase issue
     comment.
   - After `deep-innovate`, publish the innovation artifact through step 8 as an Options Phase
     issue comment.
   ...
   - After `deep-plan`, publish the plan artifact through step 8 as a Plan Phase issue comment.
...
7. If the full `deep-dive` workflow is unavailable or the selected artifact directory is under
   ...
   - Research: read repository guidance such as `AGENTS.md`, README, tests, and relevant docs;
     ...
     `research.md`, then publish it through step 8 as a Research Phase issue comment.
   - Options: compare plausible approaches and trade-offs; call out rejected approaches and why;
     write `innovate.md`, then publish it through step 8 as an Options Phase issue comment.
   - Plan: produce concrete implementation steps only when the direction is clear. If a human
     ...
     relevant; write `plan.md`, then publish it through step 8 as a Plan Phase issue comment.
