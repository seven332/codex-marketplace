...
2. Fetch issue details:
   ...
   ```
3. Create or reuse a planning directory under `/tmp/github-workflow/<short-task-name>/`.
4. Research phase:
   ...
   - Write `/tmp/github-workflow/<task>/research.md`.
5. Options phase:
   - Compare plausible approaches and trade-offs.
   - Call out rejected approaches and why.
   - Write `/tmp/github-workflow/<task>/innovate.md`.
6. Plan phase:
   ...
   - Write `/tmp/github-workflow/<task>/plan.md`.
7. Post the plan artifacts or summaries to the issue:
   ```bash
   gh issue comment <issue-number> --body-file /tmp/github-workflow/<task>/plan.md
   ```
